...
function gerarManualCompleto(analise) {
  ...
  return {
    ...
    analise_numerologica: gerarAnaliseNumerologica(nome, data_nascimento, numero_vida), // NOVA SEÇÃO
    poderes_ocultos: gerarPoderesOcultos(nome),
    arquetipos: gerarArquetipos(nome, numero_vida, signo, elemento, arquetipo),
    linguagem: gerarLinguagem(nome, numero_vida),
    rituais: gerarRituais(nome, signo, elemento),
    bloqueios: gerarBloqueios(numero_vida),
    limpeza: gerarLimpeza(nome),
    sexualidade: gerarSexualidade(nome, numero_vida, elemento),
...
function gerarAnaliseNumerologica(nome, data_nascimento, numero_vida) {
  // Função auxiliar para reduzir números
  ...
  // Cálculo do ano pessoal para 2025 e 2026
  const [dia, mes] = data_nascimento.split('/');
  
  const calcularAnoPessoal = (ano) => {
    const soma = parseInt(dia) + parseInt(mes) + ano;
    return reduzirNumero(soma);
  };

  const anoPessoal2025 = calcularAnoPessoal(2025);
  const anoPessoal2026 = calcularAnoPessoal(2026);

  const previsoes = {
    1: { foco: 'Novos começos e liderança', energia: 'Iniciativa e pioneirismo' },
    2: { foco: 'Parcerias e cooperação', energia: 'Diplomacia e paciência' },
    3: { foco: 'Criatividade e expressão', energia: 'Comunicação e arte' },
    4: { foco: 'Construção e estabilidade', energia: 'Trabalho duro e disciplina' },
    5: { foco: 'Mudanças e liberdade', energia: 'Aventura e flexibilidade' },
    6: { foco: 'Responsabilidade e cuidado', energia: 'Amor e serviço' },
    7: { foco: 'Introspecção e espiritualidade', energia: 'Estudo e meditação' },
    8: { foco: 'Conquistas materiais', energia: 'Ambição e organização' },
    9: { foco: 'Conclusões e doação', energia: 'Compaixão e liberação' }
  };

  return {
    titulo: 'Sua Análise Numerológica Completa',
    conteudo: `
${nome}, sua numerologia revela padrões únicos baseados no seu nome completo e data de nascimento.

## Seu Número de Vida: ${numero_vida}

Este é seu propósito central nesta vida - a energia que você veio desenvolver e expressar.

## Previsões Numerológicas

### 2025 - Ano Pessoal ${anoPessoal2025}
**Foco:** ${previsoes[anoPessoal2025]?.foco}
**Energia:** ${previsoes[anoPessoal2025]?.energia}

Este ano traz oportunidades para desenvolver essa área específica da sua vida. Alinhe suas ações com essa energia.

### 2026 - Ano Pessoal ${anoPessoal2026}
**Foco:** ${previsoes[anoPessoal2026]?.foco}
**Energia:** ${previsoes[anoPessoal2026]?.energia}

O próximo ano continuará expandindo os temas iniciados, com nova profundidade e maturidade.

## Como Usar Esta Informação

1. **Planeje seus projetos** alinhados com a energia do ano
2. **Desenvolva as qualidades** que cada ano pede
3. **Observe os ciclos** e flua com eles
4. **Prepare-se** para as transições entre os anos

Sua numerologia é um mapa, não um destino fixo. Use conscientemente.
    `
  };
}
...
function gerarArquetipos(nome, numero_vida, signo, elemento) {
  const arquetiposPrincipais = {
    1: ['O Líder', 'O Pioneiro', 'O Inovador'],
    2: ['O Diplomata', 'O Harmonizador', 'O Cooperador'],
    3: ['O Criativo', 'O Comunicador', 'O Artista'],
    4: ['O Construtor', 'O Organizador', 'O Alicerce'],
    5: ['O Aventureiro', 'O Explorador', 'O Liberto'],
    6: ['O Cuidador', 'O Nutrido', 'O Responsável'],
    7: ['O Sábio', 'O Místico', 'O Investigador'],
    8: ['O Realizador', 'O Líder Material', 'O Transformador'],
    9: ['O Humanitário', 'O Curador', 'O Doador'],
    11: ['O Iluminado', 'O Inspirador', 'O Visionário'],
    22: ['O Mestre Construtor', 'O Arquiteto', 'O Realizador de Sonhos'],
    33: ['O Mestre Curador', 'O Professor', 'O Servidor']
  };

  const principais = arquetiposPrincipais[numero_vida] || ['O Buscador'];

  return {
    titulo: 'Seus Arquétipos de Poder',
    conteudo: `
${nome}, baseado no seu Número de Vida ${numero_vida}, seus arquétipos principais são:

**${principais.join(', ')}**

## Como Usar Seus Arquétipos

### Arquétipos Principais
Estes são seus "superpoderes" naturais. Use-os em situações que exigem suas qualidades inatas.

### Arquétipos Situacionais
Você pode "vestir" outros arquétipos dependendo da situação:

**Para Liderança:** O Imperador, A Imperatriz
**Para Criatividade:** O Mago, O Louco
**Para Relacionamentos:** Os Enamorados, A Estrela
**Para Desafios:** A Força, O Carro
**Para Sabedoria:** O Ermitão, A Sacerdotisa
**Para Transformação:** A Morte, O Julgamento

## Ativação Prática

1. **Escolha o arquétipo** para a situação
2. **Visualize-se incorporando** suas qualidades
3. **Aja como se** já possuísse esses atributos
4. **Observe os resultados** e ajuste conforme necessário

Lembre-se: você não está limitado aos seus arquétipos principais. Eles são ferramentas que você pode usar conscientemente.
    `
  };
}

function gerarLinguagem(nome, numero_vida) {
  const afirmacoesLeves = {
    1: [
      'Eu sou luz que ilumina meu caminho',
      'Eu cocrio minha realidade com o universo',
      'Eu sou grata por todas as oportunidades',
      'Minha energia transforma espaços',
      'Eu confio na minha intuição'
    ],
    2: [
      'Eu irradio paz e harmonia',
      'Eu sou ponte entre mundos',
      'Eu cocrio com amor e compaixão',
      'Minha sensibilidade é meu dom',
      'Eu fluo com a vida naturalmente'
    ],
    // ... continuar para todos os números
  };

  return {
    titulo: 'Linguagem Vibracional de Luz',
    conteudo: `
${nome}, suas palavras são pontes entre intenção e realidade.

## Suas Afirmações de Luz

${afirmacoesLeves[numero_vida]?.map(afirmacao => `**${afirmacao}**`).join('\n') || ''}

## Como Praticar

### Manhã (5 minutos)
1. **Respire profundamente** 3 vezes
2. **Escolha uma afirmação** que ressoe com você
3. **Repita com sentimento** por 2 minutos
4. **Visualize** a energia se expandindo

### Noite (5 minutos)
1. **Reflita** sobre o dia com gratidão
2. **Escolha uma afirmação** para integração
3. **Sussurre** para seu subconsciente
4. **Durma** com essa energia

## Estados de Ondas Cerebrais

**Alpha (8-13 Hz):** Relaxamento consciente
- Meditação leve, caminhadas na natureza

**Theta (4-8 Hz):** Portal da manifestação
- Meditação profunda, visualizações

**Delta (0.5-4 Hz):** Reprogramação
- Sono profundo, cura

Para acessar **Theta** (ideal para manifestação):
1. Sente confortavelmente
2. Respire ritmicamente (4 segundos inspira, 4 expira)
3. Relaxe completamente o corpo
4. Foque em uma intenção clara
5. Mantenha por 10-20 minutos

Neste estado, suas afirmações têm poder máximo de programação.
    `
  };
}
