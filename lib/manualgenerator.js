...
const ESTILO_ELEMENTO = {
  'Fogo': {
    assinatura: 'ação, coragem, presença',
    excesso: 'impulsividade, intensidade sem direção',
    cura: 'foco + ritual de aterramento + decisão em 24h',
    neuro: 'sistema de recompensa alto; precisa de desafios curtos e metas visíveis'
  },
  'Terra': {
    assinatura: 'consistência, estrutura, realidade',
    excesso: 'controle, rigidez, medo de errar',
    cura: 'flexibilidade + exposição gradual + “feito > perfeito”',
    neuro: 'tende a evitar risco; precisa de micro-vitórias repetidas para segurança'
  },
  'Ar': {
    assinatura: 'mente rápida, comunicação, estratégia',
    excesso: 'ansiedade, hiper-análise, dispersão',
    cura: 'prioridade única + ritual de silêncio + execução em blocos',
    neuro: 'muita atividade cognitiva; precisa reduzir ruído e escolher 1 alvo por vez'
  },
  'Água': {
    assinatura: 'intuição, profundidade emocional, magnetismo',
    excesso: 'apego, oscilação, absorver energia alheia',
    cura: 'limites + descarrego emocional + rotinas de autocuidado',
    neuro: 'alta sensibilidade; precisa de regulação (respiração + escrita) para clareza'
  }
};
...
const SIGNO_PROFUNDO = {
  'Áries': { motor:'coragem + começo', risco:'impulso sem direção', amor:'admiração + verdade', dinheiro:'metas curtas', frase:'Você lidera o início.' },
  'Touro': { motor:'consistência + estabilidade', risco:'apego ao conforto', amor:'segurança + presença', dinheiro:'rotina', frase:'Profundidade sustenta.' },
  'Gêmeos': { motor:'mente rápida + comunicação', risco:'novidade > profundidade', amor:'conversa + leveza', dinheiro:'conteúdo + vendas', frase:'Escolha o essencial.' },
  'Câncer': { motor:'afeto + intuição emocional', risco:'absorver demais', amor:'cuidado recíproco', dinheiro:'limites', frase:'Sinta, mas filtre.' },
  'Leão': { motor:'presença + expressão', risco:'validar-se demais', amor:'lealdade', dinheiro:'posicionamento', frase:'O seu lugar é seu.' },
  'Virgem': { motor:'melhoria + precisão', risco:'perfeccionismo', amor:'coerência', dinheiro:'processo', frase:'Progresso > perfeito.' },
  'Libra': { motor:'harmonia + estética', risco:'indecisão', amor:'reciprocidade', dinheiro:'negociação', frase:'Escolha vira destino.' },
  'Escorpião': { motor:'verdade + transformação', risco:'controle', amor:'profundidade', dinheiro:'foco', frase:'Você renasce melhor.' },
  'Sagitário': { motor:'visão + liberdade', risco:'pular sem sustentar', amor:'verdade + leveza', dinheiro:'visão + rotina', frase:'Visão só vira realidade quando vira repetição.' },
  'Capricórnio': { motor:'estrutura + legado', risco:'peso/autocobrança', amor:'respeito', dinheiro:'método', frase:'Você constrói.' },
  'Aquário': { motor:'inovação + futuro', risco:'distância emocional', amor:'amizade + espaço', dinheiro:'comunidade', frase:'Fiel à sua visão.' },
  'Peixes': { motor:'sensibilidade + imaginação', risco:'fuga', amor:'magia com limites', dinheiro:'prazo + chão', frase:'Ancore no visível.' }
};

const REGENTE_PROFUNDO = {
  'Marte': { drive:'ação', alerta:'impulsividade', chave:'agir com direção' },
  'Vênus': { drive:'valor', alerta:'apego', chave:'escolher o que sustenta' },
  'Mercúrio': { drive:'mente', alerta:'ansiedade', chave:'clareza + execução' },
  'Lua': { drive:'segurança emocional', alerta:'oscilação', chave:'rotina de regulação' },
  'Sol': { drive:'identidade', alerta:'ego ferido', chave:'autoestima estável' },
  'Plutão/Marte': { drive:'transformação', alerta:'controle', chave:'verdade + entrega' },
  'Júpiter': { drive:'expansão', alerta:'dispersão', chave:'repetição do essencial' },
  'Saturno': { drive:'estrutura', alerta:'peso', chave:'feito > perfeito' },
  'Urano/Saturno': { drive:'inovação com trilho', alerta:'rebeldia sem sistema', chave:'ideia + rotina' },
  'Netuno/Júpiter': { drive:'imaginação', alerta:'fuga', chave:'ancorar no real' }
};

const ELEMENTO_ESTILO_VIDA = {
  'Fogo': { emDiaBom:'você puxa e contagia', emDiaRuim:'acelera por ansiedade', ajuste:'ritmo > intensidade' },
  'Terra': { emDiaBom:'você sustenta e finaliza', emDiaRuim:'trava para evitar erro', ajuste:'feito > perfeito' },
  'Ar': { emDiaBom:'você entende e estrategiza', emDiaRuim:'pensa demais e dispersa', ajuste:'prioridade única + execução' },
  'Água': { emDiaBom:'você aprofunda e cura', emDiaRuim:'absorve e se confunde', ajuste:'limites + descarga emocional' }
};

const NUMERO_PROFUNDO = {
  1: { essencia: 'autonomia e liderança', força: 'decisão e começo', sombra: 'impaciência e isolamento', cura: 'liderar com presença e ritmo' },
  2: { essencia: 'conexão e sensibilidade', força: 'harmonia e percepção', sombra: 'dependência e medo de desagradar', cura: 'limites + verdade simples' },
  3: { essencia: 'expressão e criatividade', força: 'carisma e comunicação', sombra: 'dispersão e fuga do desconforto', cura: 'rotina criativa + entrega semanal' },
  4: { essencia: 'estrutura e construção', força: 'disciplina e consistência', sombra: 'rigidez e perfeccionismo', cura: 'feito > perfeito + flexibilidade' },
  5: { essencia: 'liberdade e expansão', força: 'adaptação e coragem', sombra: 'excesso e instabilidade', cura: 'regras simples + foco em 1 canal' },
  6: { essencia: 'cuidado e responsabilidade', força: 'lealdade e presença', sombra: 'controle e auto-sacrifício', cura: 'cuidar sem se apagar' },
  7: { essencia: 'profundidade e sabedoria', força: 'análise e intuição refinada', sombra: 'isolamento e paralisia', cura: 'confiar no corpo + executar pequeno' },
  8: { essencia: 'poder e realização', força: 'resultado e visão', sombra: 'dureza e controle', cura: 'delegar + agir com estratégia' },
  9: { essencia: 'propósito e impacto', força: 'empatia e visão ampla', sombra: 'salvar todos e se perder', cura: 'limites + missão com forma' },
  11:{ essencia: 'visão e intuição elevada', força: 'inspiração e sensibilidade', sombra: 'ansiedade e sobrecarga', cura: 'aterrar e canalizar' },
  22:{ essencia: 'materialização e legado', força: 'construção grande', sombra: 'pressão e perfeccionismo', cura: 'executar simples e contínuo' },
  33:{ essencia: 'cura e amor elevado', força: 'acolhimento e transformação', sombra: 'auto-sacrifício', cura: 'amar com limites' },
};
const ARQUETIPOS = {
  MAGA: {
    nome: 'O Mago(a)',
    luz: 'transformação consciente, foco, manifestação com método',
    sombra: 'quer controlar tudo, fica mental, perde o corpo',
    uso: 'use quando precisar canalizar energia em ação simples e repetida',
    pratica: '1 tarefa por vez + 1 ritual curto + 1 entrega por dia'
  },
  ORACULO: {
    nome: 'O Oráculo',
    luz: 'intuição precisa, leitura de padrões, sensibilidade refinada',
    sombra: 'ansiedade disfarçada de “sinal”, ruído emocional',
    uso: 'use para decidir com clareza sem entrar em paranoia',
    pratica: 'fato/sensação/próximo passo (3 linhas) + decisão em 24h'
  },
  GUERREIRA: {
    nome: 'O Guerreiro(a)',
    luz: 'coragem, corte, limites, execução',
    sombra: 'reatividade, pressa, “provar valor” com esforço',
    uso: 'use para sair da procrastinação e colocar limites',
    pratica: '25 min execução + 1 “não” elegante'
  },
  AMANTE: {
    nome: 'O Amante',
    luz: 'magnetismo, prazer consciente, presença, autoestima sensorial',
    sombra: 'validação, carência, confundir intensidade com amor',
    uso: 'use para fortalecer autoestima e relação com o corpo',
    pratica: '5 min presença (respiração + postura) + 1 escolha que te honra'
  },
  RAINHA: {
    nome: 'O Rei / A Rainha',
    luz: 'auto-respeito, padrão alto, estabilidade emocional, elegância',
    sombra: 'rigidez, frieza, testar pessoas para se proteger',
    uso: 'use para manter padrão sem entrar em jogo emocional',
    pratica: 'padrões claros + consistência + limites sem explicar demais'
  },
  SABIA: {
    nome: 'O Sábio(a)',
    luz: 'verdade, síntese, maturidade, visão estratégica',
    sombra: 'isolamento, excesso de análise, “entender tudo” e não fazer',
    uso: 'use para organizar mente e transformar insight em plano',
    pratica: 'escrever 1 prioridade + 3 ações + executar a menor hoje'
  },
  CONSTRUTORA: {
    nome: 'O Construtor(a)',
    luz: 'rotina, método, consistência, materialização',
    sombra: 'perfeccionismo, controle, demora para começar',
    uso: 'use para dar chão ao seu potencial e sustentar por 30 dias',
    pratica: 'feito > perfeito + checklists + microvitórias'
  },
  CURADORA: {
    nome: 'O Curador(a)',
    luz: 'acolhimento, presença emocional, regeneração',
    sombra: 'absorver demais, salvar todo mundo, se apagar',
    uso: 'use para regular sistema nervoso e limpar energia emocional',
    pratica: 'limites + descarrego (escrita) + autocuidado diário'
  }
};

// Puxadores por elemento (via signo)
const ARQ_POR_ELEMENTO = {
  Fogo: ['GUERREIRO', 'MAGO', 'REI/RAINHA'],
  Terra: ['CONSTRUTOR/A', 'REI/RAINHA', 'SABIO'],
  Ar: ['SABIO', 'ORACULO', 'MAGO'],
  Água: ['CURADOR/A', 'AMANTE', 'ORACULO']
};

// Puxadores por número
const ARQ_POR_NUMERO = {
  1: ['GUERREIRO/A', 'REI/RAINHA'],
  2: ['CURADOR/A', 'AMANTE'],
  3: ['AMANTE', 'MAGO/A'],
  4: ['CONSTRUTORO/A', 'REI/RAINHA'],
  5: ['GUERREIRO/A', 'MAGO/A'],
  6: ['CURADORO/A', 'REI/RAINHA'],
  7: ['SABIO/A', 'ORACULO'],
  8: ['REI/RAINHA', 'CONSTRUTOR/A'],
  9: ['CURADOR/A', 'SABIO/A'],
  11: ['ORACULO', 'MAGO/A', 'SABIO/A'],
  22: ['CONSTRUTOR/A', 'REI/RAINHA', 'MAGO/A'],
  33: ['CURADOR/A', 'AMANTE', 'SABIO/A']
};

// Puxadores por objetivo (ajuste os labels para bater com os seus objetivos)
const ARQ_POR_OBJETIVO = {
  'Regulação': ['CURADOR/A', 'CONSTRUTOR/A', 'SABIO/A'],
  'Melhorar Autoestima': ['REI/RAINHA', 'AMANTE', 'GUERREIROA'],
  'Amor': ['AMANTE', 'REI/RAINHA', 'CURADORO/A'],
  'Dinheiro': ['REI/RAINHA', 'CONSTRUTOR/A', 'MAGO/A'],
  'Carreira': ['CONSTRUTOR/A', 'SABIO/A', 'GUERREIRO/A'],
  'Clareza': ['SABIO/A', 'ORACULO', 'CONSTRUTOR/A'],
  'Disciplina': ['CONSTRUTOR/A', 'REI/RAINHA', 'GUERREIRO/A'], // ← AQUI
  'Clareza e virada': ['SABIO/A', 'CURADOR/A', 'CONSTRUTOR/A'],
};
...
2) **Criando identidade** (“eu ajo como a Rainha / a Construtora”), o que aumenta constância.  
...
- Se precisa de limite → Rainha / Guerreira
- Se precisa de constância → Construtora
- Se precisa de clareza → Sábia / Oráculo
- Se precisa regular emoção → Curadora 
- Se precisa magnetismo e autoestima → Amante 
- Se precisa transformar e executar → Maga

PASSO B — Ritual curtíssimo (3 minutos)
1 minuto respirando (4–4–6)  
+ 1 frase de identidade: “Hoje eu ajo como ${a1.nome}.”  
...
const mecanismoPorObjetivo = (() => {
  ...
  const txt =
`${firstName}, este módulo é a parte que deixa o manual “escrito para você”.
Aqui não é teoria. É como você funciona na vida real.
...
Quando percebe, já está cansada, sobrecarregada ou frustrada.
...
function buildTipoPessoa({ signo, elemento, regente, numN, numArch, objPack }) {
  const e = ESTILO_ELEMENTO[elemento] || ESTILO_ELEMENTO['Ar'];
  const n = NUMERO_PROFUNDO[numN] || NUMERO_PROFUNDO[7];

  return `
Seu Tipo de Pessoa (Elemento × Número)

Você é uma combinação rara: ${elemento} + ${numN}.

• Seu elemento mostra como você reage ao mundo:
${e.assinatura}
...
• Força: ${n.força}
• Sombra: ${n.sombra}
• Caminho de evolução: ${n.cura}

Essa combinação cria um padrão invisível:

Você tende a viver em intensidade interna.
Quando alinhada → vira magnetismo.
Quando desalinhada → vira sobrecarga.

No amor:
Você precisa de constância emocional, não intensidade.

No dinheiro:
Você prospera quando repete o essencial.

Seu foco agora:
${objPack?.frase || 'Constância vira destino.'}
`;
}
...
function numeroArquetipo(n) {
  ...
  const map = {
    1: { nome: 'A Pioneira', virtude: 'liderança', sombra: 'impaciência', chave: 'decidir e sustentar' },
    2: { nome: 'A Diplomata', virtude: 'conexão', sombra: 'dependência', chave: 'limites claros' },
    3: { nome: 'A Comunicadora', virtude: 'carisma', sombra: 'dispersão', chave: 'rotina criativa' },
    4: { nome: 'A Construtora', virtude: 'disciplina', sombra: 'rigidez', chave: 'feito > perfeito' },
    5: { nome: 'A Livre', virtude: 'adaptação', sombra: 'excesso', chave: 'foco + liberdade com regras' },
    6: { nome: 'A Guardiã', virtude: 'cuidado', sombra: 'controle', chave: 'cuidar sem se perder' },
    7: { nome: 'A Investigadora', virtude: 'sabedoria', sombra: 'isolamento', chave: 'confiar e agir' },
    8: { nome: 'A Realizadora', virtude: 'prosperidade', sombra: 'dureza', chave: 'poder com coração' },
    9: { nome: 'A Curadora', virtude: 'propósito', sombra: 'salvar todos', chave: 'propósito com limites' },
    11: { nome: 'A Intuitiva Mestra', virtude: 'visão', sombra: 'ansiedade/pressão', chave: 'canalizar em rotina' },
    22: { nome: 'A Arquiteta Mestra', virtude: 'materialização', sombra: 'peso/perfeccionismo', chave: 'planejar e executar' },
    33: { nome: 'A Mestra do Amor', virtude: 'cura afetiva', sombra: 'auto-sacrifício', chave: 'amar com limites' }
  };
...
function objetivoPerfil(obj) {
 ...
 if (o.includes('ansiedad') || o.includes('emoc')) {
  return { label: 'Reduzir Ansiedade', foco: 'sistema nervoso e clareza interna' };
}

if (o.includes('autoestima')) {
  return { label: 'Melhorar Autoestima', foco: 'auto-respeito e confiança interna' };
}

if (o.includes('disciplina')) {
  return { label: 'Ter mais disciplina', foco: 'constância e execução' };
}

if (o.includes('rotina')) {
  return { label: 'Organizar minha rotina', foco: 'estrutura e eixo diário ' };
}

if (o.includes('profissional')) {
  return { label: 'Crescer profissionalmente', foco: 'posicionamento e sustentação' };
}

if (o.includes('atrair relacionamento')) {
  return { label: 'Atrair relacionamento saudável', foco: 'critério, calma e constância' };
}

if (o.includes('relacionamento atual')) {
  return { label: 'Melhorar meu relacionamento atual', foco: 'comunicação, acordo e presença' };
}
  

  // “inteligência de intenção”: mapeia objetivos comuns.
  if (o.includes('amor') || o.includes('relacion')) {
    return { label: 'Relacionamento', foco: 'constância emocional', frase: 'Amor não é intensidade. É consistência.' };
  }
  if (o.includes('dinheiro') || o.includes('prosper') || o.includes('carreira') || o.includes('trabalho')) {
    return { label: 'Prosperidade', foco: 'execução e posicionamento', frase: 'Dinheiro gosta de decisão, repetição e presença.' };
  }
  if (o.includes('ansiedad') || o.includes('emoc') || o.includes('autoestima')) {
    return { label: 'Regulação', foco: 'sistema nervoso', frase: 'Você não precisa de motivação. Precisa de regulação.' };
  }
  if (o.includes('propósito') || o.includes('missão')) {
    return { label: 'Propósito', foco: 'direção + plano', frase: 'Propósito vira realidade quando ganha agenda.' };
  }
  return { label: titleCase(obj), foco: 'direção', frase: 'Quando você escolhe um alvo, o mundo muda de resposta.' };
}
...
mas quando você começa a se abandonar no pequeno.`,
    ...
    blocks = [
      ...
      },
      {
        ...
        how: 'Você adia o simples e complica o que seria direto. Fica ocupada — mas não resolve o que realmente importa.',
...
É sustentar quando deixa de ser empolgante.`,
    ...
    blocks = [
      ...
      },
      {
        ...
        invisible: 'Perfeccionismo protege do risco de falhar e de se decepcionar consigo mesma.',
      ...
      },
      {
        ...
        effect: 'Você se sente ocupada, mas não vira consistente em nada.',
...
function buildPlan7({ elemento, numN, objPack, love, work }) {
  ...
  if (objetivo.includes('autoestima')) {
    ...
    days = [
      ...
      'Dia 2 — Quebra de resistência: faça por 15 minutos a tarefa que você vinha adiando. Objetivo do dia: provar para si mesma que você faz mesmo sem vontade.',
...
function buildLove({ elemento, numArch, lovePack, relacaoStatus, signo, firstName }) {
  ...
  if ([2, 6, 9, 33].includes(numN)) {
    stop.push('Parar de cuidar/ceder para ser escolhida.');
  ...
  const wound = (() => {
    ...
    if ([2, 6, 9, 33].includes(numN)) return 'medo de não ser escolhida (por isso você cuida demais).';
...
function buildMoney({ elemento, numArch, workPack, objPack }) {
  ...
  let archetype = 'Estratégica';
  if ([1, 8, 22].includes(numN)) archetype = 'Realizadora';
  if ([2, 6, 33].includes(numN)) archetype = 'Conectora';
  if ([3, 5].includes(numN)) archetype = 'Comunicadora';
  if ([4].includes(numN)) archetype = 'Construtora';
  if ([7, 11].includes(numN)) archetype = 'Visionária/Intuitiva';
  if ([9].includes(numN)) archetype = 'Curadora com Propósito';
...
Você vai parar de esperar a versão mais motivada de si mesma.
...
• validação (ser escolhida, elogiada, aprovada)
...
Você promete coisas para si mesma e não sustenta.
...
Quando existe química, possibilidade ou expectativa, sua mente tenta completar o resto.
Você projeta intenção onde ainda existe só abertura.
Projeta profundidade onde ainda existe só intensidade.
E se vincula antes de existir base.

Isso não acontece por fraqueza.
Acontece porque o emocional tenta antecipar segurança.

Mas antecipação emocional sem consistência cria vínculo frágil.`,
...
• se apega ao potencial e não ao padrão
• tolera ambiguidades que depois te drenam
• percebe sinais, mas espera confirmação emocional`,

  emotionalPattern:
`No fundo, existe medo de perder uma possibilidade boa, de parecer exigente demais ou de “estragar” algo com cobrança.

Então você flexibiliza cedo demais.

Mas flexibilidade sem critério vira autoabandono.
E autoabandono destrói atração saudável.`,
  ...
  blocks: [
    {
      name: 'Confundir química com compatibilidade',
      appears: 'você sente conexão e já dá peso emocional alto.',
      invisible: 'O cérebro se apega ao pico e ignora consistência.',
      effect: 'você se envolve antes de ter base real.'
    },
    {
      name: 'Tolerar ambiguidade',
      appears: 'você aceita “vamos ver”, sinais mistos e pouca clareza.',
      invisible: 'Ambiguidade mantém esperança viva.',
      effect: 'você fica presa sem perceber.'
    },
    {
      name: 'Baixar padrão quando sente carência',
...
• transforma visão em movimento visível`,
  ...
  blocks: [
    ...
    },
    {
      ...
      appears: 'você espera estar pronta demais para aparecer.',
...
Crescimento não vem de pensar mais. Vem de sustentar melhor.`
 ...
 if (obj.includes('rotina')) {
  return wrap({
    ...
    realLife: `• você abre o dia sem uma direção clara\n• responde o urgente antes do importante\n• começa várias coisas e termina poucas\n• sente que fez muito, mas avançou pouco\n• tenta se reorganizar no fim do dia, quando já está cansada`,
    ...
    blocks: [
      ...
      },
      {
        ...
        effect: 'Você termina o dia cansada, com sensação de dívida interna, mesmo tendo feito muita coisa.'
      }
    ],
...
• promete coisas para si mesma e, quando não sustenta, perde confiança em si  
...
Vem de se tornar alguém confiável para si mesma.
...
function buildDiagnosticoProfundo(ctx) {
  ...
  const dados = [
    birthFmt ? `Você nasceu em ${birthFmt}${local ? `, em ${titleCase(local)}` : ''}.` : '',
    birthFmt ? `Isso não é só dado, é configuração energética.` : ''
  ].filter(Boolean).join('\n');

  const introBase = clampStr(
`${firstName}, este manual foi feito para ser um espelho que não distorce, e depois virar um mapa que organiza.

A sua combinação ${signo} (${elemento} — ${regente}) + ${numN} não cria apenas um perfil.
Cria um campo interno específico, um jeito próprio de sentir, decidir, reagir e se movimentar pela vida.

${dados ? `${dados}\n\n` : ''}Seu signo (${signo}) te move por ${s.motor}.
Risco: ${s.risco}.
Frase do seu signo: “${s.frase}”

Regência (${regente}): força = ${r.drive} | alerta = ${r.alerta} | chave = ${r.chave}.

Seu elemento (${elemento}) traz ${e.assinatura}.
O desafio: ${e.excesso}.
A cura: ${e.cura}.
Neuro lembrete: ${e.neuro}.

Seu número (${numN}) ativa ${n.essencia}.
Sua força: ${n.forca}.
Sua sombra: ${n.sombra}.
Chave evolutiva: ${n.cura}.
`, 4000);
  ...
  return clampStr(`${introBase}\n\n${diagnosticoObjetivo}`, 9000);
}
