...
function buildCalendarExtra60({ elemento, objPack }) {
  // Continuação do calendário — semanas 5 a 13 (dias 31 a 90).
  // Usado só quando o order bump "Calendário Estendido 90 dias" é comprado.
  const e = ESTILO_ELEMENTO[elemento] || ESTILO_ELEMENTO['Ar'];
  const objLine = objPack?.frase ? `Objetivo do ciclo: ${objPack.frase}` : '';

  const weekTemplates = [
    {
      week: 'Semana 5 — Renovação (revisão do ciclo)',
      focus: `Fechar o primeiro mês e ajustar rota sem abandonar o que já funcionou.`,
      days: [
        'Dia 31: revisar o que mudou no último mês (3 linhas)',
        'Dia 32: ajustar 1 hábito que não pegou',
        'Dia 33: reconhecer 1 vitória pequena (sem minimizar)',
        'Dia 34: 1 conversa de alinhamento (amor ou trabalho)',
        'Dia 35: organizar o próximo ciclo de 30 dias',
        'Dia 36: ritual de renovação (3 min)',
        'Dia 37: descanso ativo — sem culpa'
      ]
    },
    {
      week: 'Semana 6 — Disciplina (rotina como identidade)',
      focus: `Rotina sustenta o que a motivação não consegue sozinha.`,
      days: [
        'Dia 38: definir horário fixo pra 1 tarefa-chave',
        'Dia 39: repetir o horário fixo (dia 2)',
        'Dia 40: cortar 1 desculpa recorrente',
        'Dia 41: repetir o horário fixo (dia 3)',
        'Dia 42: revisar progresso da rotina',
        'Dia 43: ajustar o que está pesado demais',
        'Dia 44: celebrar 7 dias de constância'
      ]
    },
    {
      week: 'Semana 7 — Conexão (relações e comunicação)',
      focus: `O que não se fala vira ruído. Clareza aproxima.`,
      days: [
        'Dia 45: 1 mensagem de reconexão (quem você anda evitando)',
        'Dia 46: expressar 1 necessidade real, sem rodeios',
        'Dia 47: ouvir sem preparar resposta',
        'Dia 48: 1 limite claro numa relação',
        'Dia 49: agradecer alguém importante',
        'Dia 50: revisar 1 padrão repetitivo nos vínculos',
        'Dia 51: tempo de qualidade (sem tela) com quem importa'
      ]
    },
    {
      week: 'Semana 8 — Abundância (dinheiro em movimento)',
      focus: `Dinheiro circula quando você posiciona valor com clareza.`,
      days: [
        'Dia 52: revisar 1 gasto que não faz sentido',
        'Dia 53: 1 ação de receita (oferta, cobrança, contato)',
        'Dia 54: organizar 1 número financeiro real',
        'Dia 55: 1 negociação ou pedido direto',
        'Dia 56: guardar/investir 1 valor simbólico',
        'Dia 57: revisar crenças de escassez (3 linhas)',
        'Dia 58: planejar 1 meta financeira de 30 dias'
      ]
    },
    {
      week: 'Semana 9 — Cura (sombra e integração)',
      focus: `O que você evita olhar continua no comando até ser visto.`,
      days: [
        'Dia 59: nomear 1 medo específico (sem dramatizar)',
        'Dia 60: escrever a origem de 1 padrão de autossabotagem',
        'Dia 61: 1 ato de autocompaixão real',
        'Dia 62: perdoar (ou soltar) 1 situação antiga',
        'Dia 63: ritual de integração da sombra (5 min)',
        'Dia 64: revisar o que mudou desde o dia 1',
        'Dia 65: descanso emocional — sem cobrança'
      ]
    },
    {
      week: 'Semana 10 — Visão (metas de médio prazo)',
      focus: `Sem direção, energia se dispersa. Escolha 1 foco de verdade.`,
      days: [
        'Dia 66: escrever a meta principal dos próximos 90 dias',
        'Dia 67: quebrar a meta em 3 passos concretos',
        'Dia 68: 1 ação do passo 1',
        'Dia 69: eliminar 1 distração que compete com a meta',
        'Dia 70: 1 ação do passo 2',
        'Dia 71: revisar e ajustar o plano',
        'Dia 72: visualizar o resultado pronto (3 min)'
      ]
    },
    {
      week: 'Semana 11 — Ação (execução sem desculpas)',
      focus: `Ideia sem execução é só entretenimento. Hora de mover.`,
      days: [
        'Dia 73: executar o passo mais adiado',
        'Dia 74: entregar uma versão imperfeita',
        'Dia 75: pedir feedback de alguém confiável',
        'Dia 76: ajustar com base no feedback',
        'Dia 77: 1 ação de exposição (mostrar, publicar, oferecer)',
        'Dia 78: revisar ritmo — acelerar ou sustentar?',
        'Dia 79: celebrar a entrega, não só o resultado'
      ]
    },
    {
      week: 'Semana 12 — Presença (corpo, energia, intuição)',
      focus: `Sem energia, nenhuma estratégia se sustenta.`,
      days: [
        'Dia 80: 1 ajuste de sono ou alimentação',
        'Dia 81: movimento físico (mesmo que 10 min)',
        'Dia 82: 10 min de silêncio sem estímulo',
        'Dia 83: checar 1 sinal do corpo que você ignora',
        'Dia 84: ritual de conexão com a intuição',
        'Dia 85: reduzir 1 excesso (tela, ruído, compromisso)',
        'Dia 86: gratidão pelo próprio corpo'
      ]
    },
    {
      week: 'Semana 13 — Fechamento dos 90 Dias',
      focus: `Integrar o ciclo inteiro e decidir o que continua.`,
      days: [
        'Dia 87: revisar os 90 dias (3 vitórias + 3 aprendizados)',
        'Dia 88: encerrar o que não serve mais',
        'Dia 89: escolher 1 hábito que fica para sempre',
        'Dia 90: celebrar o ciclo e planejar o próximo'
      ]
    }
  ];

  const weeks = weekTemplates.map((w) => ({
    week: w.week,
    focus: clampStr(`${w.focus}${objLine ? ` • ${objLine}` : ''}`, 500),
    days: w.days
  }));
...
function buildTarotFallback({ firstName, carta, invertida, signo, elemento, numN, numArch, objPack }) {
  const s = SIGNO_PROFUNDO[signo] || SIGNO_PROFUNDO['Sagitário'];
  const nBase = NUMERO_PROFUNDO[numN] || NUMERO_PROFUNDO[7] || {};
  const significadoBase = invertida ? (carta?.significadoInvertido || carta?.significado) : carta?.significado;
  const palavras = Array.isArray(carta?.palavrasChave) ? carta.palavrasChave.join(', ').toLowerCase() : '';
  const objetivoLabel = objPack?.label || 'clareza e direção';
  const objetivoFoco  = objPack?.foco  || 'decisão e constância';
  const arquName = (numArch?.nome || `Número ${numN}`).trim();
  const nomeCarta = carta?.nome || 'Esta carta';

  return clampStr(
`${firstName}, ${nomeCarta}${invertida ? ' invertida' : ''} apareceu agora como espelho, não como acaso.
${significadoBase || 'O que ela revela diz respeito ao momento que você está atravessando.'}

Em ${signo}${elemento ? ` (${elemento})` : ''}, isso ganha um contorno específico: ${s?.motor || 'sua forma de agir e sentir'} entra em contato direto com ${palavras || 'o tema desta carta'}. O ponto de atenção de ${signo}, que é ${s?.risco || 'o excesso que te desequilibra'}, é justamente o que essa carta está pedindo para você olhar de frente.

Com o Número ${numN} (${arquName}), a mensagem se aprofunda: ${nBase?.essencia || 'sua essência'} encontra ${nBase?.sombra || 'um padrão que se repete'}, e a saída passa por ${nBase?.cura || 'presença e consistência'}.

No seu objetivo de ${objetivoLabel}, essa carta não é decoração. Ela está te mostrando exatamente onde focar: ${objetivoFoco}.`,
    2200
  );
}

// ─────────────────────────────────────────────────────────────────────────────
// buildDiagnosticoCtx — resolve o contexto simbólico a partir de um row do banco
// Aceita tanto snake_case (Supabase) quanto camelCase (params do generateManual).
// Usado pelo webhook para montar o ctx antes de chamar gerarDiagnosticoIA.
// ─────────────────────────────────────────────────────────────────────────────
// ─────────────────────────────────────────────────────────────────────────────
// buildAmorCtx — resolve o contexto afetivo para gerarAmorIA
// ─────────────────────────────────────────────────────────────────────────────
// ─────────────────────────────────────────────────────────────────────────────
// buildTipoPessoaCtx — resolve o contexto comportamental para gerarTipoPessoaIA
// ─────────────────────────────────────────────────────────────────────────────
export function buildTipoPessoaCtx(row) {
  const signo    = titleCase(row.signo || '');
  const numN     = toLifePath(row.numero_vida ?? row.numeroVida);
  const elemento = ELEMENTO_SIGNO[signo]     || 'Ar';
  const regente  = REGENTE_SIGNO[signo]      || '';
  const s  = SIGNO_PROFUNDO[signo]      || {};
  const r  = REGENTE_PROFUNDO[regente]  || {};
  const n  = NUMERO_PROFUNDO[numN]      || {};
  const ev = ELEMENTO_ESTILO_VIDA[elemento] || {};
  const objetivo = row.objetivo_principal || row.objetivoPrincipal || '';
  const op = objetivoPerfil(objetivo);
  const lp = loveFrame(row.relacao_status  || row.relacaoStatus  || '');
  const wp = workFrame(row.trabalho_status || row.trabalhoStatus || '');
  return {
    ...
    sombraNum:     n.sombra  || '',
    curaNum:       n.cura    || '',
    emDiaBom:      ev.emDiaBom  || '',
    emDiaRuim:     ev.emDiaRuim || '',
    ajuste:        ev.ajuste    || '',
    objetivo,
    objetivoLabel: op.label || objetivo,
    objetivoFoco:  op.foco  || '',
    relacaoStatus: row.relacao_status  || row.relacaoStatus  || '',
    trabalhoStatus:row.trabalho_status || row.trabalhoStatus || '',
    loveContexto:  lp.contexto || '',
    workContexto:  wp.contexto || '',
...
// ─────────────────────────────────────────────────────────────────────────────
// buildPlano7Ctx — resolve o contexto para gerarPlano7IA
// ─────────────────────────────────────────────────────────────────────────────
export function buildPlano7Ctx(row) {
  const signo    = titleCase(row.signo || '');
  const numN     = toLifePath(row.numero_vida ?? row.numeroVida);
  const elemento = ELEMENTO_SIGNO[signo] || 'Ar';
  const n  = NUMERO_PROFUNDO[numN]     || {};
  const e  = ESTILO_ELEMENTO[elemento] || {};
  const objetivo = row.objetivo_principal || row.objetivoPrincipal || '';
  const op = objetivoPerfil(objetivo);
  const lp = loveFrame(row.relacao_status  || row.relacaoStatus  || '');
  const wp = workFrame(row.trabalho_status || row.trabalhoStatus || '');
...
// ─────────────────────────────────────────────────────────────────────────────
// buildArquetiposCtx — resolve o contexto para gerarArquetiposIA
// ─────────────────────────────────────────────────────────────────────────────
// ─────────────────────────────────────────────────────────────────────────────
// buildPontoCegoCtx — contexto para gerarPontoCegoIA
// ─────────────────────────────────────────────────────────────────────────────
// ─────────────────────────────────────────────────────────────────────────────
// buildDinheiroCtx — contexto para gerarDinheiroIA
// ─────────────────────────────────────────────────────────────────────────────
export function buildDinheiroCtx(row) {
  const signo    = titleCase(row.signo || '');
  const numN     = toLifePath(row.numero_vida ?? row.numeroVida);
  const elemento = ELEMENTO_SIGNO[signo] || 'Ar';
  const s  = SIGNO_PROFUNDO[signo]     || {};
  ...
  return {
    firstName:     pickFirstName(row.nome || ''),
    signo, elemento, numN,
    dinheiroSigno: s.dinheiro   || '',
    sombraNum:     n.sombra     || '',
    curaNum:       n.cura       || '',
    assinatura:    e.assinatura || '',
    excesso:       e.excesso    || '',
    cura:          e.cura       || '',
    objetivo:      row.objetivo_principal || row.objetivoPrincipal || '',
    trabalhoStatus:row.trabalho_status || row.trabalhoStatus || '',
    workContexto:  wp.contexto || '',
    workMeta:      wp.meta     || '',
    signoMarte:      row.signo_marte      || null,
...
// ─────────────────────────────────────────────────────────────────────────────
// buildRituaisCtx — contexto para gerarRituaisIA
// ─────────────────────────────────────────────────────────────────────────────
export function buildRituaisCtx(row) {
  const signo    = titleCase(row.signo || '');
  const numN     = toLifePath(row.numero_vida ?? row.numeroVida);
  const elemento = ELEMENTO_SIGNO[signo] || 'Ar';
  const n = NUMERO_PROFUNDO[numN]     || {};
...
export function buildPontoCegoCtx(row) {
  const signo    = titleCase(row.signo || '');
  const numN     = toLifePath(row.numero_vida ?? row.numeroVida);
  const elemento = ELEMENTO_SIGNO[signo] || 'Ar';
  const s = SIGNO_PROFUNDO[signo]  || {};
...
// ─────────────────────────────────────────────────────────────────────────────
// buildBloqueiosCtx — contexto para gerarBloqueiosIA
// ─────────────────────────────────────────────────────────────────────────────
export function buildBloqueiosCtx(row) {
  const signo    = titleCase(row.signo || '');
  const numN     = toLifePath(row.numero_vida ?? row.numeroVida);
  const elemento = ELEMENTO_SIGNO[signo] || 'Ar';
  const s  = SIGNO_PROFUNDO[signo]     || {};
...
export function buildArquetiposCtx(row) {
  const signo    = titleCase(row.signo || '');
  const numN     = toLifePath(row.numero_vida ?? row.numeroVida);
  const elemento = ELEMENTO_SIGNO[signo] || 'Ar';
  const s = SIGNO_PROFUNDO[signo]  || {};
...
export function buildAmorCtx(row) {
  const signo    = titleCase(row.signo || '');
  const numN     = toLifePath(row.numero_vida ?? row.numeroVida);
  const elemento = ELEMENTO_SIGNO[signo] || 'Ar';
  const s        = SIGNO_PROFUNDO[signo]  || {};
  const lp       = loveFrame(row.relacao_status || row.relacaoStatus || '');
  const anoPessoal = row.ano_pessoal != null ? Number(row.ano_pessoal) : null;
  return {
    firstName:    pickFirstName(row.nome || ''),
    signo,
    elemento,
    ...
    signoNodo:     row.signo_nodo     || null,
    anoPessoal,
    anoPessoalEssencia: anoPessoal ? (NUMERO_PROFUNDO[anoPessoal]?.essencia || '') : '',
    loveContexto:  lp.contexto || '',
    loveRisco:    lp.risco    || '',
    loveMeta:     lp.meta     || '',
  };
...
export function buildObjetivoCtx(row) {
  const signo    = titleCase(row.signo || '');
  const numN     = toLifePath(row.numero_vida ?? row.numeroVida);
  const elemento = ELEMENTO_SIGNO[signo] || 'Ar';
  const s        = SIGNO_PROFUNDO[signo]  || {};
...
export function buildLeituraCtx(row) {
  const signo    = titleCase(row.signo || '');
  const numN     = toLifePath(row.numero_vida ?? row.numeroVida);
  const elemento = ELEMENTO_SIGNO[signo]     || 'Ar';
  const s        = SIGNO_PROFUNDO[signo]     || {};
...
export function buildCalendarioCtx(row) {
  const signo    = titleCase(row.signo || '');
  const numN     = toLifePath(row.numero_vida ?? row.numeroVida);
  const elemento = ELEMENTO_SIGNO[signo]     || 'Ar';
  const e        = ESTILO_ELEMENTO[elemento] || {};
...
export function buildFechamentoCtx(row) {
  const signo = titleCase(row.signo || '');
  const numN  = toLifePath(row.numero_vida ?? row.numeroVida);
  const s     = SIGNO_PROFUNDO[signo] || {};
  const n     = NUMERO_PROFUNDO[numN] || {};
...
export function buildDiagnosticoCtx(row) {
  const signo    = titleCase(row.signo || '');
  const numN     = toLifePath(row.numero_vida ?? row.numeroVida);
  const elemento = ELEMENTO_SIGNO[signo]     || 'Ar';
  const regente  = REGENTE_SIGNO[signo]      || '';
  ...
  const r = REGENTE_PROFUNDO[regente] || {};
  const n = NUMERO_PROFUNDO[numN]     || {};
  const numeroAlma      = row.numero_alma      != null ? Number(row.numero_alma)      : null;
  const numeroExpressao = row.numero_expressao != null ? Number(row.numero_expressao) : null;
  const anoPessoal      = row.ano_pessoal      != null ? Number(row.ano_pessoal)      : null;
  return {
    firstName:     pickFirstName(row.nome || ''),
    signo,
    ...
    numeroExpressao,
    anoPessoal,
    almaEssencia:       numeroAlma      ? (NUMERO_PROFUNDO[numeroAlma]?.essencia      || '') : '',
    expressaoEssencia:  numeroExpressao ? (NUMERO_PROFUNDO[numeroExpressao]?.essencia || '') : '',
    anoPessoalEssencia: anoPessoal      ? (NUMERO_PROFUNDO[anoPessoal]?.essencia      || '') : '',
  };
}

export function buildMapaCtx(row) {
  const signo           = titleCase(row.signo || '');
  const numN            = toLifePath(row.numero_vida ?? row.numeroVida);
  const numeroAlma      = row.numero_alma      != null ? Number(row.numero_alma)      : null;
  const numeroExpressao = row.numero_expressao != null ? Number(row.numero_expressao) : null;
  ...
  return {
    ...
    numeroExpressao,
    anoPessoal,
    numNEssencia:        NUMERO_PROFUNDO[numN]?.essencia              || '',
    almaEssencia:        numeroAlma      ? (NUMERO_PROFUNDO[numeroAlma]?.essencia      || '') : '',
    expressaoEssencia:   numeroExpressao ? (NUMERO_PROFUNDO[numeroExpressao]?.essencia || '') : '',
    anoPessoalEssencia:  anoPessoal      ? (NUMERO_PROFUNDO[anoPessoal]?.essencia      || '') : '',
  };
...
const relacaoStatus = norm(params.relacaoStatus);
const trabalhoStatus = norm(params.trabalhoStatus);

const signoLuaParam        = norm(params.signoLua)        || null;
const signoVenusParam      = norm(params.signoVenus)      || null;
const signoMarteParam      = norm(params.signoMarte)      || null;
const signoNodoParam       = norm(params.signoNodo)       || null;
const signoMercurioParam   = norm(params.signoMercurio)   || null;
const signoAscendenteParam = norm(params.signoAscendente) || null;
const anoPessoalParam      = params.anoPessoal      != null ? Number(params.anoPessoal)      : null;
const numeroAlmaParam      = params.numeroAlma      != null ? Number(params.numeroAlma)      : null;
const numeroExpressaoParam = params.numeroExpressao != null ? Number(params.numeroExpressao) : null;

const lovePack = loveFrame(relacaoStatus);
const workPack = workFrame(trabalhoStatus);
...
const moneyUAU = getMoneyPersonalizadoUAU({
  ...
  // COVER
  const coverMapaAstral = [
    `Sol: ${signo || '—'}`,
    signoAscendenteParam ? `Ascendente: ${signoAscendenteParam}` : null,
    signoLuaParam        ? `Lua: ${signoLuaParam}`               : null,
    signoMercurioParam   ? `Mercúrio: ${signoMercurioParam}`     : null,
    signoVenusParam      ? `Vênus: ${signoVenusParam}`           : null,
    signoMarteParam      ? `Marte: ${signoMarteParam}`           : null,
    signoNodoParam       ? `Nodo Norte: ${signoNodoParam}`       : null,
    `Número de Vida: ${numN || '—'} • Arquétipo: ${numArch?.nome || '—'}`,
    (numeroAlmaParam || numeroExpressaoParam || anoPessoalParam)
      ? [
          numeroAlmaParam      ? `Alma: ${numeroAlmaParam}`           : null,
          numeroExpressaoParam ? `Expressão: ${numeroExpressaoParam}` : null,
          anoPessoalParam      ? `Ano Pessoal: ${anoPessoalParam}`    : null,
        ].filter(Boolean).join(' • ')
      : null,
  ].filter(Boolean);
  ...
  // MAPA COMPLETO — legenda estática + síntese IA
  {
    const legendItems = [];

    // Sol
    legendItems.push(`Sol em ${signo} → sua identidade central: ${SIGNO_PROFUNDO[signo]?.motor || 'visão e expansão'}`);

    // Ascendente
    if (signoAscendenteParam) {
      legendItems.push(`Ascendente em ${signoAscendenteParam} → sua máscara social: ${SIGNO_PROFUNDO[signoAscendenteParam]?.chave || 'como os outros te veem'}`);
    }

    // Lua
    if (signoLuaParam) {
      legendItems.push(`Lua em ${signoLuaParam} → seu padrão emocional: ${SIGNO_PROFUNDO[signoLuaParam]?.amor || 'padrão emocional automático'}`);
    }

    // Mercúrio
    if (signoMercurioParam) {
      legendItems.push(`Mercúrio em ${signoMercurioParam} → sua mente: ${SIGNO_PROFUNDO[signoMercurioParam]?.chave || 'como pensa e se comunica'}`);
    }

    // Vênus
    if (signoVenusParam) {
      legendItems.push(`Vênus em ${signoVenusParam} → seu amor: ${SIGNO_PROFUNDO[signoVenusParam]?.amor || 'como ama e o que atrai'}`);
    }

    // Marte
    if (signoMarteParam) {
      legendItems.push(`Marte em ${signoMarteParam} → sua ação: ${SIGNO_PROFUNDO[signoMarteParam]?.risco || 'como age sob pressão'}`);
    }

    // Nodo Norte
    if (signoNodoParam) {
      legendItems.push(`Nodo Norte em ${signoNodoParam} → sua direção de crescimento: ${SIGNO_PROFUNDO[signoNodoParam]?.frase || 'o que precisa integrar nesta vida'}`);
    }

    // Número de Vida
    const numVidaNome = (numArch?.nome || `Número ${numN}`).trim().replace(/^[^\w]+/, '');
    const numVidaEss  = NUMERO_PROFUNDO[numN]?.essencia || '';
    legendItems.push(`Número de Vida ${numN} → sua missão: ${numVidaNome}${numVidaEss ? ` — ${numVidaEss}` : ''}`);

    // Alma
    if (numeroAlmaParam) {
      legendItems.push(`Alma ${numeroAlmaParam} → sua motivação interna: ${NUMERO_PROFUNDO[numeroAlmaParam]?.essencia || 'o que quer de verdade por dentro'}`);
    }

    // Expressão
    if (numeroExpressaoParam) {
      legendItems.push(`Expressão ${numeroExpressaoParam} → seu destino visível: ${NUMERO_PROFUNDO[numeroExpressaoParam]?.essencia || 'o que veio construir no mundo'}`);
    }

    // Ano Pessoal
    if (anoPessoalParam) {
      legendItems.push(`Ano Pessoal ${anoPessoalParam} → seu ciclo atual: ${NUMERO_PROFUNDO[anoPessoalParam]?.essencia || 'momento de transição'}`);
    }

    // Síntese IA
    let mapaSintese = null;
    const rawSintese = params.sinteseGerada;
    if (rawSintese) {
      try {
        const parsedS = typeof rawSintese === 'string' ? JSON.parse(rawSintese) : rawSintese;
        if (parsedS?.body) mapaSintese = parsedS.body;
      } catch (_) { /* null */ }
    }
  ...
  // TEXT: Diagnóstico — usa conteúdo IA se disponível, fallback estático caso contrário
  let diagnosticoBody;
  const rawDiagnostico = params.diagnosticoGerado;
  if (rawDiagnostico) {
    try {
      const parsed = typeof rawDiagnostico === 'string'
        ? JSON.parse(rawDiagnostico)
        : rawDiagnostico;
      if (parsed?.combinacao && parsed?.conflito_central && parsed?.diagnostico_objetivo) {
        diagnosticoBody = [
          parsed.combinacao,
          parsed.conflito_central,
          parsed.diagnostico_objetivo,
          parsed.frase_diagnostico ? `"${parsed.frase_diagnostico}"` : '',
        ].filter(Boolean).join('\n\n');
      }
    } catch (_) { /* JSON inválido → fallback */ }
  }
  if (!diagnosticoBody) {
    diagnosticoBody = buildDiagnosticoProfundo(ctx);
  }
...
sections.push({
  ...
  // TIPO DE PESSOA — usa IA se disponível, fallback estático
  let tipoPessoaBody = moduloExtra;
  const rawTipoPessoa = params.tipoPessoaGerado;
  if (rawTipoPessoa) {
    try {
      const parsedTP = typeof rawTipoPessoa === 'string' ? JSON.parse(rawTipoPessoa) : rawTipoPessoa;
      if (typeof parsedTP?.body === 'string' && parsedTP.body.trim().length > 100) {
        tipoPessoaBody = parsedTP.body.replace(/\n{3,}/g, '\n\n');
      }
    } catch (_) { /* fallback */ }
  }
  ...
  // ARQUÉTIPOS — usa IA se disponível, fallback estático
  let arquetiposSection = moduloExtraArquetipos;
  const rawArquetipos = params.arquetiposGerado;
  if (rawArquetipos) {
    try {
      const pa = typeof rawArquetipos === 'string' ? JSON.parse(rawArquetipos) : rawArquetipos;
      if (pa?.dominante?.nome && pa?.emocional?.nome && pa?.sombra?.nome) {
        const fmt = (tipo, label) =>
          `${label} — ${tipo.nome}\n${tipo.descricao}\n"${tipo.frase}"`;
        const body = [
          fmt(pa.dominante, 'Arquétipo Dominante'),
          '',
          fmt(pa.emocional, 'Arquétipo Emocional'),
          '',
          fmt(pa.sombra,    'Arquétipo Sombra'),
        ].join('\n');
        arquetiposSection = { type: 'text', title: ' Arquétipos ', body };
      }
    } catch (_) { /* fallback */ }
  }
  sections.push(arquetiposSection);

  // OBJETIVO DO CICLO — usa IA se disponível, fallback estático
  let objetivoTitle = objective.title;
  let objetivoBody  = objective.body;
  const rawObjetivo = params.objetivoGerado;
  if (rawObjetivo) {
    try {
      const po = typeof rawObjetivo === 'string' ? JSON.parse(rawObjetivo) : rawObjetivo;
      if (po?.body) {
        objetivoBody  = po.body;
        if (po.title) objetivoTitle = po.title;
      }
    } catch (_) { /* fallback */ }
  }
  sections.push({ type: 'text', title: objetivoTitle, body: objetivoBody });

  // LEITURA INTEGRADA — usa IA se disponível, fallback estático
  let leituraBody = personalidade;
  const rawLeitura = params.leituraGerada;
  if (rawLeitura) {
    try {
      const pl = typeof rawLeitura === 'string' ? JSON.parse(rawLeitura) : rawLeitura;
      if (typeof pl?.body === 'string' && pl.body.trim().length > 100) {
        leituraBody = pl.body;
      }
    } catch (_) { /* fallback */ }
  }
  sections.push({ type: 'text', title: ' Leitura integrada ', body: leituraBody });

  // PONTO CEGO — usa IA se disponível, fallback estático
  let pontoCegoBody = blindSpot;
  const rawPC = params.pontoCegoGerado;
  if (rawPC) {
    try {
      const ppc = typeof rawPC === 'string' ? JSON.parse(rawPC) : rawPC;
      if (ppc?.body) pontoCegoBody = ppc.frase ? `${ppc.body}\n\n"${ppc.frase}"` : ppc.body;
    } catch (_) { /* fallback */ }
  }
  sections.push({ type: 'text', title: ' Ponto cego ', body: pontoCegoBody });

  // BLOQUEIOS — usa IA se disponível, fallback estático
  let bloqueiosData = { note: blocks.note, items: blocks.items };
  const rawBloqueios = params.bloqueiosGerado;
  if (rawBloqueios) {
    try {
      const pb = typeof rawBloqueios === 'string' ? JSON.parse(rawBloqueios) : rawBloqueios;
      if (Array.isArray(pb?.items) && pb.items.length >= 3) bloqueiosData = pb;
    } catch (_) { /* fallback */ }
  }
  sections.push({ type: 'bullets', title: ' Bloqueios ', note: bloqueiosData.note, items: bloqueiosData.items });

  // PLAN7 — usa IA se disponível, fallback estático
  let plan7Data = plan7;
  const rawPlano7 = params.plano7Gerado;
  if (rawPlano7) {
    try {
      const parsedP7 = typeof rawPlano7 === 'string' ? JSON.parse(rawPlano7) : rawPlano7;
      if (Array.isArray(parsedP7?.days) && parsedP7.days.length >= 7) plan7Data = parsedP7;
    } catch (_) { /* fallback */ }
  }
  ...
  // RITUAIS — usa IA se disponível, fallback estático
  let rituaisBody = buildRituals({ elemento, objPack });
  const rawRituais = params.rituaisGerado;
  if (rawRituais) {
    try {
      const pr = typeof rawRituais === 'string' ? JSON.parse(rawRituais) : rawRituais;
      if (Array.isArray(pr?.rituals) && pr.rituals.length >= 3) {
        rituaisBody = pr.rituals.map((r, i) => [
          `Ritual ${i + 1} — ${r.nome}`,
          r.quando ? `Quando usar: ${r.quando}` : '',
          Array.isArray(r.passos) ? r.passos.map((p, j) => `${j + 1}. ${p}`).join('\n') : '',
          r.frase ? `"${r.frase}"` : '',
        ].filter(Boolean).join('\n')).join('\n\n');
      }
    } catch (_) { /* fallback */ }
  }
  sections.push({ type: 'text', title: ' Rituais ', body: rituaisBody });

  // LOVE — usa conteúdo IA se disponível, fallback no buildLove estático
  let loveData = love;
  const rawAmor = params.amorGerado;
  if (rawAmor) {
    try {
      const parsedAmor = typeof rawAmor === 'string' ? JSON.parse(rawAmor) : rawAmor;
      if (parsedAmor?.pattern && Array.isArray(parsedAmor?.whatToStop) && Array.isArray(parsedAmor?.whatToStart)) {
        loveData = parsedAmor;
      }
    } catch (_) { /* fallback */ }
  }
  ...
  sections.push({
    type: 'love',
    title: ' Amor',
    headline: loveData.headline || love.headline,
    pattern: loveData.pattern,
    whatToStop: loveData.whatToStop,
    whatToStart: loveData.whatToStart,
    microScript: loveData.microScript || love.microScript,
  });

  // MONEY — usa IA se disponível, fallback estático
  let dinheiroData = {
  ...
  const rawDinheiro = params.dinheiroGerado;
  if (rawDinheiro) {
    try {
      const pd = typeof rawDinheiro === 'string' ? JSON.parse(rawDinheiro) : rawDinheiro;
      if (pd?.headline && Array.isArray(pd?.blocks) && Array.isArray(pd?.actions)) {
        dinheiroData = {
          ...dinheiroData,
          headline:   pd.headline,
          blocks:     pd.blocks,
          actions:    pd.actions,
          microHabits: Array.isArray(pd.microHabits) ? pd.microHabits : dinheiroData.microHabits,
        };
      }
    } catch (_) { /* fallback */ }
  }
  ...
  sections.push({
    type: 'money',
    title: ' Dinheiro',
    headline: dinheiroData.headline,
    subheadline: '',
    profile: moneyUAU.profile,
    objectiveBridge: moneyUAU.objectiveBridge,
    blocks: dinheiroData.blocks,
    actions: dinheiroData.actions,
    microHabits: dinheiroData.microHabits,
    sprint7: dinheiroData.sprint7,
  });

  // CALENDÁRIO 30 DIAS — usa IA se disponível, fallback estático
  let calendarioWeeks = calendar.weeks;
  const rawCalendario = params.calendarioGerado;
  if (rawCalendario) {
    try {
      const pc = typeof rawCalendario === 'string' ? JSON.parse(rawCalendario) : rawCalendario;
      if (Array.isArray(pc?.weeks) && pc.weeks.length >= 4) calendarioWeeks = pc.weeks;
    } catch (_) { /* fallback */ }
  }
  // Order bump "Calendário Estendido 90 dias" — sempre acrescenta os 60 dias
  // extras (semanas 5–13), independente do primeiro mês vir da IA ou estático.
  if (params.bumpCalendario90) {
    calendarioWeeks = calendarioWeeks.concat(buildCalendarExtra60({ elemento, objPack }).weeks);
  }
  sections.push({
    type: 'calendar30',
    title: params.bumpCalendario90 ? ' Calendário Estendido (90 dias) ' : ' Calendário ',
    weeks: calendarioWeeks,
  });

  // CARTA DE TAROT — usa IA se disponível, fallback personalizado caso contrário
  const rawCarta = params.cartaTarot;
  const rawTarotInterp = params.cartaTarotInterpretacao;
  if (rawCarta) {
    try {
      const carta = typeof rawCarta === 'string' ? JSON.parse(rawCarta) : rawCarta;
      let interpTitulo = null;
      let interpBody = null;
      if (rawTarotInterp) {
        try {
          const pi = typeof rawTarotInterp === 'string' ? JSON.parse(rawTarotInterp) : rawTarotInterp;
          interpTitulo = pi?.titulo || null;
          interpBody   = pi?.body   || null;
        } catch (_) { /* JSON inválido — cai no fallback abaixo */ }
      }
      if (!interpBody) {
        interpBody = buildTarotFallback({
          firstName, carta, invertida: !!carta?.invertida, signo, elemento, numN, numArch, objPack,
        });
      }
      sections.push({
        type: 'tarot',
    ...
    } catch (_) { /* sem carta — seção omitida */ }
  }

  // FECHAMENTO — usa IA se disponível, fallback estático
  let fechamentoBody   = buildClosing({ elemento, objPack });
  let fechamentoMantra = null;
  const rawFechamento  = params.fechamentoGerado;
  if (rawFechamento) {
    try {
      const pf = typeof rawFechamento === 'string' ? JSON.parse(rawFechamento) : rawFechamento;
      if (pf?.body) {
        fechamentoBody   = pf.body;
        fechamentoMantra = pf.mantra || null;
      }
    } catch (_) { /* fallback */ }
  }
  sections.push({
    ...
    title: ' Fechamento ',
    body: fechamentoBody,
    ...(fechamentoMantra ? { mantra: fechamentoMantra } : {}),
  });
...
export function renderManualMarkdown(manual) {
  ...
  manual.sections.forEach((s, idx) => {
    ...
    if (type === 'mapa') {
      lines.push('### Legenda do Mapa');
      safeArr(s.items).forEach((it) => lines.push(`- ${norm(it)}`));
      lines.push('');
      if (s.sintese) {
        lines.push('### Síntese Integrada');
        lines.push(norm(s.sintese));
        lines.push('');
      }
      lines.push('---');
      lines.push('');
