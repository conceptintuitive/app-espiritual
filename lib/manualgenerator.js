...
export function objetivoPerfil(obj) {
  const o = lower(obj);
...
export function buildTipoPessoaCtx(row) {
  ...
  return {
    ...
    resumoAnterior: row.__resumoAnterior || '',
    signo,
    elemento,
    regente,
    numN,
...
const moneyUAU = getMoneyPersonalizadoUAU({
  ...
  // DIAGNÓSTICO — usa blocos estruturados da IA se disponível, fallback estático em texto corrido
  let diagnosticoBlocks = null;
  let diagnosticoQuote = null;
  let diagnosticoBody;
  ...
  if (rawDiagnostico) {
    try {
      ...
      if (parsed?.combinacao && parsed?.conflito_central && parsed?.diagnostico_objetivo) {
        diagnosticoBlocks = [
          { label: 'A combinação', text: parsed.combinacao },
          { label: 'O conflito central', text: parsed.conflito_central },
          { label: 'Diagnóstico do seu objetivo', text: parsed.diagnostico_objetivo },
        ];
        diagnosticoQuote = parsed.frase_diagnostico || null;
      }
    } catch (_) { /* JSON inválido → fallback */ }
  }
  if (!diagnosticoBlocks) {
    diagnosticoBody = buildDiagnosticoProfundo(ctx);
  }

sections.push(diagnosticoBlocks ? {
  type: 'diagnostico',
  title: ' Diagnóstico ',
  blocks: diagnosticoBlocks,
  quote: diagnosticoQuote,
} : {
  type: 'text',
  title: ' Diagnóstico ',
  ...
  // ARQUÉTIPOS — usa cards estruturados da IA se disponível, fallback estático em texto corrido
    ...
    try {
      ...
      if (pa?.dominante?.nome && pa?.emocional?.nome && pa?.sombra?.nome) {
        arquetiposSection = {
          ...
          title: ' Arquétipos ',
          items: [
            { key: 'dominante', label: 'Arquétipo Dominante', icon: '👑', ...pa.dominante },
            { key: 'emocional', label: 'Arquétipo Emocional', icon: '💗', ...pa.emocional },
            { key: 'sombra',    label: 'Arquétipo Sombra',    icon: '🌑', ...pa.sombra },
          ],
        };
      }
    } catch (_) { /* fallback */ }
  ...
  // RITUAIS — usa cards estruturados da IA se disponível, fallback em texto corrido
  let rituaisItems = null;
  ...
  sections.push({ type: 'calendar30', title: ' Calendário ', weeks: calendarioWeeks });
  ...
  return;
    ...
    if (type === 'diagnostico') {
      safeArr(s.blocks).forEach((b) => {
        lines.push(`### ${norm(b.label)}`);
        lines.push(norm(b.text));
        lines.push('');
      });
      if (s.quote) {
        lines.push(`> "${norm(s.quote)}"`);
        lines.push('');
      }
      return;
    }
    ...
    if (type === 'archetypes') {
      safeArr(s.items).forEach((it) => {
        lines.push(`### ${norm(it.label)} — ${norm(it.nome)}`);
        lines.push(norm(it.descricao));
        lines.push('');
        if (it.frase) lines.push(`> "${norm(it.frase)}"`);
        lines.push('');
      });
      return;
    }
    ...
    if (type === 'rituals') {
      safeArr(s.items).forEach((r, i) => {
        lines.push(`### Ritual ${i + 1} — ${norm(r.nome)}`);
        if (r.quando) lines.push(`**Quando usar:** ${norm(r.quando)}\n`);
        safeArr(r.passos).forEach((p, j) => lines.push(`${j + 1}. ${norm(p)}`));
        lines.push('');
        if (r.frase) lines.push(`> "${norm(r.frase)}"`);
        lines.push('');
      });
      return;
    }
