...
// Síntese e Diagnóstico geram primeiro (ver app/api/gerar-manual-completo/route.js);
// as demais seções recebem, via resumoAnterior, uma proibição explícita do
// tema central já usado (não só o texto citado — citar sozinho reforça a
// palavra-chave em vez de afastar dela) pra não convergir no mesmo insight em
// 5-6 seções diferentes do mesmo manual. O texto já vem formatado por
// montarResumoAnterior() no orquestrador.
function blocoAntiRepeticao(resumoAnterior) {
  if (!resumoAnterior) return '';
  return `\n\n=== EVITAR REPETIÇÃO ENTRE SEÇÕES ===\n${resumoAnterior}`;
}

// Reposicionamento de "previsão" pra "autorreflexão estruturada": o risco de
// afirmar fato externo específico (evento, data, nome) é que ele pode não se
// confirmar — e é isso que gera queixa/descrédito, não a falta de precisão.
// Mantém a exigência de especificidade do mecanismo comportamental (que já é
// forte nestes prompts) — só tira o que é verificável e pode falhar.
const REGRA_SEM_PREVISAO = `
REGRA DE PREVISÃO: nunca afirme como certo um evento externo específico e verificável ("você vai conhecer alguém", "vai receber uma proposta", "vai se casar", datas exatas, nomes, resultados financeiros concretos). Descreva sempre o padrão comportamental e emocional real desta combinação — o mecanismo que se repete, não o evento que vai acontecer no mundo.`;

// Blocos de texto corrido muito longos são ruins pra leitura em mobile (a
// maioria do tráfego). Não muda a contagem de palavras já pedida em cada
// campo — só força a quebra em parágrafos menores dentro desse total.
const REGRA_PARAGRAFO_CURTO = `
REGRA DE PARÁGRAFO: em qualquer campo de texto corrido, quebre a cada ~80 palavras em um novo parágrafo (\\n\\n), formando parágrafos de 3-4 linhas. Nunca escreva um bloco único e longo sem quebra — pense em como o texto vai aparecer numa tela de celular.`;
...
export async function gerarTipoPessoaIA(ctx) {
  ...
  const {
    firstName, resumoAnterior, signo, elemento, regente, numN,
    motor, risco,
    rDrive, rChave,
...
REGRA DE PONTUAÇÃO: não use travessão (—) em nenhum momento do texto. Use vírgula, ponto final ou reformule a frase para não precisar dele.
${REGRA_SEM_PREVISAO}
${REGRA_PARAGRAFO_CURTO}
...
}${blocoAntiRepeticao(resumoAnterior)}`;
...
export async function gerarPlano7IA(ctx) {
  ...
  const {
    firstName, resumoAnterior, signo, elemento, numN,
...
O ponto cego emerge da tensão entre a sombra interna (o padrão que a pessoa repete sem perceber), o excesso do elemento quando perde o eixo, e o que resiste integrar. Não centre a leitura em como a pessoa reage num conflito com outra pessoa — isso já é o foco de outras seções (Diagnóstico, Amor). Foque em como o padrão aparece sozinha, na relação da pessoa consigo mesma. Não cite planetas pelo nome.
...
Objetivo atual: ${objetivo || 'não informado'}
Status amor: ${relacaoStatus || 'não informado'} | Trabalho: ${trabalhoStatus || 'não informado'}${nodoDesc ? `\nO que precisa integrar: "${nodoDesc}"` : ''}

INSTRUÇÃO:
O ponto cego desta combinação nasce da interação entre:
1. A sombra interna: "${sombraNum}" — o que se repete sem perceber
2. O excesso do elemento quando perde o eixo: "${excesso}" — o que acontece por dentro, sozinha, antes de qualquer interação com outra pessoa
...
Base: a sombra do número ${numN} ("${sombraNum}")${nodoDesc ? ` + o que resiste integrar ("${nodoDesc}")` : ''} — não uma reação a conflito com outra pessoa, e sim o adiamento que você faz sozinha.
...
Os comportamentos devem ser observáveis e específicos para esta combinação — não adjetivos abstratos como "você fica ansiosa". Mostre o que muda no dia a dia, no trabalho, nas decisões cotidianas. Não centre o "fora do eixo" em como a pessoa reage num conflito com outra pessoa — isso já é o foco de outras seções (Diagnóstico, Amor). Foque em hábitos, ritmo e comportamento sozinha ou em contextos não-relacionais.
...
Bloco 2 — QUANDO SAI DO EIXO: o que muda na prática, derivado do excesso de ${elemento} ("${excesso}"). Os sinais observáveis de que você perdeu o centro no trabalho e na rotina — não como reage num conflito específico com outra pessoa.
...
export async function gerarFechamentoIA(ctx) {
  ...
  const {
    firstName, resumoAnterior, signo, numN,
