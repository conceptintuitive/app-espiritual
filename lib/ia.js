...
// as demais seções recebem um resumo do que essas duas já disseram, pra não
// convergir no mesmo insight central (ex: "racionaliza em vez de sentir") em
// 5-6 seções diferentes do mesmo manual.
function blocoAntiRepeticao(resumoAnterior) {
  if (!resumoAnterior) return '';
  return `\n\nJÁ FOI DITO em outras seções deste manual (não repita a mesma ideia central, metáfora ou virada de frase — escolha um ângulo diferente para esta seção específica):\n"${resumoAnterior}"`;
}
